...
class GitHubCodeExtractor(ComponentBase):
    ...
    def run(self):
        ...
        def write_to_csv(data, output_table_path):
            ...
            column_order = ['repo_name', 'file_path', 'filename', 'language', 'code', 'url']
            df = df[column_order]
        ...
        # Create output table
        output_table = self.create_out_table_definition(f"{owner}-{repo}-repodata.csv", primary_key=['file_path'], column_names=['repo_name', 'file_path', 'filename', 'language', 'code', 'url'])
